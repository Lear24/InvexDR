function [beta_estimated,objective_value] = ...
        opt_DHDR(beta_initial,X,Y,m,n,p,d,T_max,K_max,lambda,varargin)
...
coef_v = cell(m,1);coef_inital = cell(m,1);coef_update = cell(m,1);
...
for T = 1:T_max
    ...
    for mi=1:m
        ...
        Yi=cell2mat(Y(mi));
        n_i = n(mi);
        alpha = beta_initial{mi};
        alphanew = coef_update{mi};
        v = coef_v{mi};
        ...
        for K = 1:K_max
            if (T==1) && (K==1)
                ...
                % beta_cell = coef_v;beta_cell{mi} = coef_v{mi}; % 不会更快 估计也没有什么区别
                [~ ,grad]= obj_DHDR(v,beta_cell,m,n_i,Yi,Xi,lambda,mode);
                alpha =alphanew;
        ...
        coef_inital(mi) = mat2cell(alpha,p,d);
    ...
    if abs(sum(objGiNew)-sum(objGi))<0.00005 
        fprintf('在轮次%d时, 目标函数值cost为 %f \n',T,sum(objGiNew));
...
%         objective_value(T+20:end) = [];
    ...
    elseif sum(objGiNew)-sum(objGi)>0.03 && T >= 2
        fprintf('在轮次%d时, 损失函数增加,目标函数值cost为 %f \n',T,sum(objGi)); % 保留更新之前的结果
        fprintf('算法收敛,总共通讯了%d轮\n',T);
        beta_estimated = beta_initial;
        objective_value(T+1:end) =  sum(objGi);
        % objective_value(T+20:end) = [];
        break;
    % elseif sum(objGiNew)-sum(objGi)>0.03 && T < 3
    %     step = step/2;
    %     beta_estimated = beta_initial;
    %     objective_value(T+1) = sum(objGiNew);
    else
        % continue loop-----------
        objGi=objGiNew;
