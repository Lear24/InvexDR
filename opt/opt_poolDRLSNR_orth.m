function [beta_estimated,trsimi,objall,fnorm,stop] = ...
    opt_poolDRLSNR_orth(beta_initial,X,Y,X_all_matrix,Y_all_matrix,m,n,p,d,T_max,K_max,lambda,N_max,real_beta,varargin)
% stop: 0 - max iter; 1 - convergence; (ignore the increase case)
% beta_initial: all node with one same beta;
% X,Y 
% X_all_matrix with p-by-N; Y_all_matrix with N-by-1;
...
while (N<=N_max) && (stop ==0)
    ...
    [beta_estimated,objective_value] = ...
        opt_poolDHDR_orth(beta_initial,hX,hY,m,n,p,d,T_max,K_max,lambda,optionsOPT);
    % 记录机制
