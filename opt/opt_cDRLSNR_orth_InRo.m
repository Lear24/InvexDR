function [beta_estimated,trsimi,objall,results,stop] = ...
    opt_cDRLSNR_orth_InRo(beta_initial,X,Y,m,n,p,d,T_max,K_max,lambda,N_max,real_beta,varargin)
...
while (N<=N_max) && (stop ==0)
    ...
    [beta_estimated,objective_value] = ...
        opt_cDHDR_orth(beta_initial,hX,hY,m,n,p,d,T_max,K_max,lambda,optionsOPT);
    ...
    if (abs(objall(N-1,1) - objall(N,1) ) < 0.0001)
        ...
        objall(N+1:end,:) = [];
        break;
%     elseif  (objall(N,1) - objall(N-1,1) >0.3) && N>3 %若目标函数值增加
