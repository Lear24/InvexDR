%% The main experiment with n varies: ex2-pi8
%% Setting parameters
clear all;
clc
rng('default')
...
%% 
% load('SimuV2_varn_ex2pi8_rep100.mat')
for rep = 1:repetition
    for var = 1:num_var

        rng(rep);
        % 生成噪声
        nn = (variable(var))*ones(m,1);
    ...
    if rep ==5, save('SimuV2_varn_ex2pi8_rep5.mat'), end
    if rep ==10, save('SimuV2_varn_ex2pi8_rep10.mat'), end

    save('SimuV2_varn_ex2pi8_rep100.mat')
end
...
% saveas(1,'simuv2_varn_ex2pi8_Tr.png')
% saveas(2,'simuv2_varn_ex2pi8_F.png')
...
boxplot(dTr(1:6,:)', 1:6);
colors = [0,0,0; 0,0.8,0; 1,1,0.2; 0.7,0.1,0.1; 0.5,0.2,0.5; 0,0.45,0.75];
...
xticklabels({'InvexDR', 'MAVE', 'NR-B', 'NR-O', 'pNR-B','pNR-O'});
ylabel('tr$\left\{\mathbf{P}(\widehat{\mathbf{\beta}})\mathbf{P}(\widehat{\mathbf{\beta^*}})\right\}$', ...
    'FontSize', 16,'Interpreter','latex');
...
% saveas(3,'simuv2_varn_ex2pi8_dTr.png')
% saveas(4,'simuv2_varn_ex2pi8_dF.png')