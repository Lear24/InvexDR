%% The ain experiment with sd varies: ex1-pi8
%% Setting parameters
clear all;
clc
rng('default')
...
% 真实参数生成
theta_1=0;theta_2=pi/8;
[real_beta,real_beta_similarity]= ...
    generate_RandomCoef(m,p,d,theta_1,theta_2,coef);


%% Sample from the dirichlet to obtain the distribution of a sample
rng(5)
N = 300*m; num_var = 3;
variable = [0.6, 0.7, 0.8];
n_all = zeros(m,num_var);
r_all = zeros(m,num_var);
for i = 1:num_var

alpha = variable(i);
r = util_drchrnd([alpha,1-alpha],m);
n_all(:,i) = round((N-1200)*r(:,1)/sum(r(:,1)));
...
% % ylim([0,200])
% plot([1,m],[N/m,N/m],'--r','LineWidth',1.5)
% hold on
% % for i = 1:num_var
% for i = 1:num_var
% 
% plot(1:m,sort(n_all(:,i))','LineWidth',2)
% 
% end 
% 
% legend('$n_i = N/m$','$r = 0.6$','$r = 0.7$',...
%     '$r = 0.8$','Location','best','FontSize', 12,'Interpreter','latex')
% 
% title('Sorted Sample size Per Node', 'FontSize', 16,'Interpreter','latex');
% xlabel('Node (Sorted)', 'FontSize', 16,'Interpreter','latex');
% ylabel('Sample Size', 'FontSize', 16,'Interpreter','latex');
...
%% 
% load('SimuV2_varsd_ex1pi8_rep100.mat')
for rep = 1:repetition
    for var = 1:num_var

        rng(rep);
        % 生成噪声
        nn = round(n_all(:,var)*0.5);
    ...
    if rep ==5, save('SimuV2_varsd_ex1pi8_rep5.mat'), end
    if rep ==10, save('SimuV2_varsd_ex1pi8_rep10.mat'), end

    save('SimuV2_varsd_ex1pi8_rep100.mat')
end

%%
% clear
clc
% load('SimuV2_varn_ex1pi8_rep100.mat')

% 计算tr
repetition = 1; ind_invex = 4;
EvalTr = zeros(12,num_var);EvalF = zeros(12,num_var);
distri_Tr= zeros(12,m);  distri_F = zeros(12,m);
for method = 1
    if method == 6, continue; end
    for var = 1:num_var % [3,4,5]
